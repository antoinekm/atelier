...
export function renderCapabilityRequestGuide(
  ...
  return [
    ...
    '- Credential / account access you cannot self-provision (e.g. a Stripe key, a paid account): {"type":"request_credential","payload":{"envKey":"STRIPE_SECRET_KEY","service":"stripe","scope":"<optional>","reason":"<why>","howToObtain":"<NUMBERED steps (1., 2., 3., ...), each one action, each click step carrying the exact full https:// URL and the exact scopes; end with: paste the value into this approval>","browserAgentPrompt":"<a self-contained prompt the board can paste into a browser agent like Claude for Chrome to perform the acquisition and return the value>"}}. ALWAYS write howToObtain as a NUMBERED list of steps (1., 2., 3., ...): each step is a single concrete action, and any step that needs a click MUST carry the exact full https:// URL to the precise page (not "the dashboard"), e.g. "1. Open https://dashboard.stripe.com/apikeys  2. Click Create restricted key  3. Enable these permissions: <list them>  4. Copy the key and paste it into this approval". Never write a vague prose paragraph and never omit the links or the exact scopes; the human must be able to follow it click by click without searching. Likewise write browserAgentPrompt as concrete end-to-end steps a browser-driving agent can execute (which exact URL to open, exactly what to create/configure with which scopes, and to report back the resulting value). The board provides the value; on approval it is injected into your run environment as $envKey (read it from your shell), never returned in plaintext.',
    isLead === false
